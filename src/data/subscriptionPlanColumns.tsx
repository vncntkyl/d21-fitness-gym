...
export const columns: ColumnDef<Subscription>[] = [
    ...
    },
    {
        ...
        cell: ({ row, column }) => {
            ...
            return <Badge variant={mode === "inactive" ? "destructive" : "outline"} className={cn(mode === "inactive" ? "uppercase bg-red-500 border-red-600 font-lexend" : "uppercase bg-emerald-500 border-emerald-600 text-white font-lexend")}>{mode}</Badge>
        },
        filterFn: (row, columnId, filterValue: string[]) => {
            const options = arrayMap[columnId as keyof typeof arrayMap]
            const values = filterValue.map(filterVal => {
