...
export default function StackedDragCarousel({
  ...
  const cards = useMemo(() => {
    return subscriptions.filter((option) => option.duration !== 6);
  }, []);
  const handleNext = () => {
  ...
  return (
    <div className="relative w-full max-w-lg h-70 mx-auto mt-1">
      {cards.map((card, i) => {
        ...
        return (
          <AnimatePresence key={card.duration}>
            <motion.div
